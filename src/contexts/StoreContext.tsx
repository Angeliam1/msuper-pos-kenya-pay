...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { currentTenant, getCurrentTenantId } = useMultiTenant();
  const [currentStore, setCurrentStore] = useState<StoreLocation | null>(null);
  const [stores, setStores] = useState<StoreLocation[]>([]);

  // Store-specific data storage with tenant isolation
  ...
  // Initialize stores for current tenant
  useEffect(() => {
    if (currentTenant) {
      // Mock stores for the current tenant - in real implementation, fetch from database
      const tenantStores: StoreLocation[] = [
        {
          id: `store-${currentTenant.id}-1`,
          name: `${currentTenant.name} - Main Branch`,
      ...
      setCurrentStore(tenantStores[0]);

      // Initialize store data for tenant stores
      const initialStoreData: Record<string, any> = {};
      tenantStores.forEach(store => {
        initialStoreData[store.id] = {
          products: [],
          customers: [{
            id: 'walk-in',
  ...
  return (
    <StoreContext.Provider value={{
      ...
      stores,
      addStore,
      updateStore: (id: string, updates: Partial<StoreLocation>) => {
        setStores(prev => prev.map(store => 
          store.id === id ? { ...store, ...updates } : store
        ));
      },
      ...
      updateStoreProduct: (storeId: string, productId: string, updates: Partial<Product>) => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
      ...
      addProductToStore: (storeId: string, product: Omit<Product, 'id'>) => {
        const newProduct: Product = {
          ...product,
          id: `product-${Date.now()}`,
          createdAt: new Date(),
      ...
      addCustomerToStore: (storeId: string, customer: Omit<Customer, 'id'>) => {
        const newCustomer: Customer = {
          ...customer,
          id: `customer-${Date.now()}`,
          createdAt: new Date()
      ...
      updateStoreCustomer: (storeId: string, customerId: string, updates: Partial<Customer>) => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
      ...
      addTransactionToStore: (storeId: string, transaction: Transaction) => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
      ...
      addAttendantToStore: (storeId: string, attendant: Omit<Attendant, 'id'>) => {
        const newAttendant: Attendant = {
          ...attendant,
          id: `attendant-${Date.now()}`,
          createdAt: new Date()
      ...
      updateStoreCashBalance: (storeId: string, amount: number, type: 'add' | 'subtract') => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
      ...
      updateStoreSettings: (storeId: string, settings: any) => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
      ...
      getStoreSuppliers: (storeId: string) => {
        return storeData[storeId]?.suppliers || [];
      },
      addSupplierToStore: (storeId: string, supplier: any) => {
        const newSupplier = {
          ...supplier,
          id: `supplier-${Date.now()}`,
          createdAt: new Date()
      ...
      updateStoreSupplier: (storeId: string, supplierId: string, updates: any) => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
      ...
      deleteStoreSupplier: (storeId: string, supplierId: string) => {
        setStoreData(prev => ({
          ...prev,
          [storeId]: {
            ...prev[storeId],
